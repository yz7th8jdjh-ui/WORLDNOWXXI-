...
if(pkg.dependencies?.["maplibre-gl"]!=="6.4.1"){
  run("npm install maplibre-gl@6.4.1 --legacy-peer-deps");
}
...
import {Map,NavigationControl,FullscreenControl,Marker,Popup} from "maplibre-gl";
  ...
  useEffect(()=>{
    ...
    for(const item of visible){
      ...
      const marker=new Marker({element:el})
        ...
        .setPopup(new Popup({offset:22}).setDOMContent(popup))
        .addTo(map);
